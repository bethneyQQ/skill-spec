...
@cli.command("lint")
@click.argument("name")
@click.option("--fix", is_flag=True, help="Reformat and fix the YAML file")
@click.option("--format", "output_format", type=click.Choice(["text", "json"]),
              default="text", help="Output format")
def lint(name: str, fix: bool, output_format: str):
    """Validate and optionally fix YAML syntax in spec.yaml.

    This command checks for YAML parsing errors and validates the structure
    through Pydantic models. Use --fix to reformat the file using canonical
    YAML serialization, which fixes indentation and formatting issues.

    This is useful when spec.yaml was manually edited or generated by an AI
    that may have introduced formatting inconsistencies.
    """
    ...
    # Step 1: Check if file can be parsed as YAML
    try:
        ...
        errors.append({
            ...
            "suggestion": "Check indentation and YAML syntax. Common issues: "
                          "inconsistent indentation, missing colons, unquoted special chars."
        })
        if output_format == "json":
            ...
            click.secho("YAML Lint: FAILED", fg="red", bold=True)
            click.echo()
            for err in errors:
                click.secho(f"Error: {err['message']}", fg="red")
                click.echo(f"  Suggestion: {err['suggestion']}")
        sys.exit(1)
    ...
    # Step 2: Validate through Pydantic model (optional for --fix)
    spec = None
    schema_valid = True
    try:
        spec = SkillSpec.model_validate(spec_data)
    except Exception as e:
        schema_valid = False
        warnings.append({
            "type": "schema_validation",
            "message": f"Schema validation warning: {e}",
            "suggestion": "Run 'skillspec validate' for detailed schema errors"
        })

    # Step 3: Apply fix if requested
    if fix:
        if schema_valid and spec:
            # Use Pydantic model for canonical serialization
            canonical_yaml = spec.to_yaml()
        else:
            # Fallback: re-serialize the raw data with yaml.dump
            # This fixes basic formatting issues without full schema validation
        ...
        spec_path.write_text(canonical_yaml, encoding="utf-8")
        if output_format == "json":
            click.echo(json.dumps({
                "valid": True,
                "fixed": True,
                ...
                "errors": [],
                "warnings": warnings if not schema_valid else [],
            }, indent=2))
        else:
            click.secho("YAML Lint: FIXED", fg="green", bold=True)
            click.echo(f"  Reformatted: {spec_path}")
            if not schema_valid:
                click.secho("  Note: Schema validation has issues. Run 'skillspec validate' for details.", fg="yellow")
        sys.exit(0)

    # Step 4: Check for potential formatting issues
    if schema_valid and spec:
        canonical_yaml = spec.to_yaml()
        original_lines = content.strip().split('\n')
        canonical_lines = canonical_yaml.strip().split('\n')

        if len(original_lines) != len(canonical_lines):
            warnings.append({
                "type": "formatting",
                "message": f"Line count differs: {len(original_lines)} vs {len(canonical_lines)} (canonical)",
                "suggestion": "Use --fix to reformat"
            })
    ...
    if output_format == "json":
        ...
        click.secho("YAML Lint: PASSED", fg="green", bold=True)
        click.echo("  YAML syntax is valid and parseable.")
        if warnings:
            click.echo()
            ...
            for w in warnings:
                ...
                if w.get('suggestion'):
                    click.echo(f"    Suggestion: {w['suggestion']}")
...
def _generate_skill_md(spec_data: dict) -> str:
    ...
    # Description (Anthropic required section, was "Purpose")
    lines.append("## Description")
    lines.append("")
    lines.append(skill.get("purpose", ""))
    lines.append("")

    # When to Use (Anthropic recommended section)
    use_when = triggers_section.get("use_when", [])
    do_not_use_when = triggers_section.get("do_not_use_when", [])
    if use_when or do_not_use_when:
        ...
        if do_not_use_when:
            ...
            for condition in do_not_use_when:
                lines.append(f"- {condition}")
            lines.append("")
    ...
    # Limitations (Anthropic required section, was "What This Skill Does NOT Do")
    if non_goals:
    ...
    # Instructions (Anthropic required section, was "Workflow")
    if steps:
    ...
    # Error Codes (Anthropic recommended section, was "Error Handling")
    if failure_modes:
    ...
    # Examples (Anthropic recommended section)
    ...
    if examples:
        ...
        for ex in examples:
            ...
            if ex.get("trigger"):
                lines.append(f"**Trigger**: {ex['trigger']}")
            lines.append("")
            if ex.get("input"):
                lines.append("**Input**:")
                lines.append("```json")
                lines.append(json.dumps(ex["input"], indent=2))
                lines.append("```")
                lines.append("")
            if ex.get("output"):
                lines.append("**Output**:")
    ...
    # Related Skills (Anthropic recommended section, was "Works Well With")
